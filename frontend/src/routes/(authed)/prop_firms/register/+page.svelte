<script lang="ts">
    ...
    const platformTypes = ["MT4", "MT5", "cTrader"];

    let isSubmitting = false;
    let error: string | null = null;

    // Set available_balance equal to full_balance when full_balance changes
    function updateAvailableBalance() {
        form.available_balance = form.full_balance;
    }
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            const numericFormData = {
                ...form,
                full_balance:
                    typeof form.full_balance === "string"
                        ? parseFloat(form.full_balance)
                        : form.full_balance,
                available_balance:
            ...
            if (!response.ok) {
                throw new Error(
                    `Failed to create prop firm: ${response.statusText}`,
                );
            }
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-3">
                    <div>
                        ...
                        <input
                            ...
                            on:change={updateAvailableBalance}
                            step="0.01"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                        />
                    </div>
