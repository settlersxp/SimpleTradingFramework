...
def upgrade():
    ...
    if context.has_table('trades'):
        with op.batch_alter_table('trades', schema=None) as batch_op:
            # Check if the column 'platform_id' exists
            if not context.has_column('trades', 'id'):
                batch_op.add_column(
                    sa.Column('id', sa.Integer(), nullable=False))
            if not context.has_column('trades', 'strategy'):
                batch_op.add_column(
                    sa.Column('strategy', sa.String(length=100), nullable=False))
            if not context.has_column('trades', 'order_type'):
                batch_op.add_column(
                    sa.Column('order_type', sa.String(length=10), nullable=False))
            if not context.has_column('trades', 'contracts'):
                batch_op.add_column(
                    sa.Column('contracts', sa.Float(), nullable=False))
            if not context.has_column('trades', 'ticker'):
                batch_op.add_column(
                    sa.Column('ticker', sa.String(length=20), nullable=False))
            if not context.has_column('trades', 'position_size'):
                batch_op.add_column(
                    sa.Column('position_size', sa.Float(), nullable=False))
            if not context.has_column('trades', 'created_at'):
                batch_op.add_column(
