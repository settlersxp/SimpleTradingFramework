...
def downgrade():
    ...
    if context.has_table('prop_firms'):
        with op.batch_alter_table('prop_firms', schema=None) as batch_op:
            ...
            if context.has_column('prop_firms', 'platform_type'):
                batch_op.drop_column('platform_type')
            if context.has_column('prop_firms', 'port'):
                batch_op.drop_column('port')
            if context.has_column('prop_firms', 'ip_address'):
                batch_op.drop_column('ip_address')
            if context.has_column('prop_firms', 'password'):
                batch_op.drop_column('password')
            if context.has_column('prop_firms', 'username'):
                batch_op.drop_column('username')
            if context.has_column('prop_firms', 'is_active'):
